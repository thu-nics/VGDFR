...
class CausalConv3d(nn.Module):
    ...
    def forward(self, x):
        ...
        # print(f"Downsample: {x_shape} -> {self.conv.stride} -> {o.size()}")
        return o
...
class UpsampleCausal3D(nn.Module):
    ...
    """
    def extra_repr(self):
        return f"upsample_factor={self.upsample_factor}"
    ...
    def forward(
        ...
        if self.interpolate:
            ...
            # print(f"Upsample: {hidden_states.size()} -> {self.upsample_factor} -> {torch.cat((first_h, other_h), dim=2).size()}")
            if T > 1:
